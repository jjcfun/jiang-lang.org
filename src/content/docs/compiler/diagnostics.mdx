...
`!` marks the mutability of one layer. On the outermost layer of a binding declaration, it marks the
binding slot, so the binding must use outer `!` before it can be reassigned:
...
For arrays, the element layer and outer binding slot are separate:
