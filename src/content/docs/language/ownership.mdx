...
Borrowing `struct` and `union` types declare their public lifetime regions with `@region`. Every
declared region must be used by a field or union payload. A reference has an outer borrow plus the
