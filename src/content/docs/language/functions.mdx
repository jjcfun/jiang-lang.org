...
cannot outlive the source owner. A unique `T&!` does not cross directly; it can reborrow as `T&`,
which freezes writes through the original unique reference while that shared reborrow is live. Raw
pointers are not automatically `Sendable`.
