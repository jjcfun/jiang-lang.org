...
Use `async call(...)` or `async { ... }` to start work eagerly and receive an address-stable
...
Use `new async call(...)` or `new async { ... }` when the Task owner must independently move, be
returned, be passed to another function, or be stored in a movable field or container. The result is
a movable, non-copyable `Task<T>^`; moving it transfers only the owner pointer and never relocates
the running Task:
