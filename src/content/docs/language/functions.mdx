...
`unsafe` and `async` can be written before the return type inside `Fn<...>` to mark the context
required to call the function. They describe the outer function type, not the return value itself:
...
`Fn<async Bool>[]&` means a borrowed slice whose elements are async function pointers. To describe
an async function that returns a borrowed slice, write:
