...
`public const` exports a compile-time value through the module interface. `comptime { ... }` is a
top-level compile-time block for source selection; see [Const and Comptime](/language/const-and-comptime/).
