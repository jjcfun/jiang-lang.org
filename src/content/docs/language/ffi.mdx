...
Indexing `T*` is unsafe because it carries no length. Wrap it with a length field or use `T[]&` when
Jiang code needs a borrowed length-bearing view. The old `T[*]` spelling is compatibility syntax
