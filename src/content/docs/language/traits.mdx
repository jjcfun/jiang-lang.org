...
Dynamic trait values use three compiler-provided companion types on every trait:
...
`Writer`. Construction is a checked trait operation on the trait:
...
Jiang 0.4.1 supports dynamic dispatch for ref receiver trait instance methods and vtable slot
access. Traits with `@self(move)` requirements are rejected for `Any`, `VTable`, and `Receiver`;
owned receiver trait objects are planned for a later release.
