...
| `value$.ptr()` | returns a `T*` single-object raw pointer; requires `unsafe` |
...
Low-level memory operations are gated by the `unsafe` effect. Use `do [unsafe] { ... }` when a
