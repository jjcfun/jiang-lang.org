...
Fixed arrays use one aggregate place capability. `T[N]` is readonly and `T[N]!` allows both element
writes and replacement of the complete array. Jiang does not define a separate `T![N]` form:
...
Raw-pointer indexing is unsafe because the pointer carries no length. Use `T[]&` when Jiang code
needs a borrowed length-bearing view. The old `T[*]` spelling is compatibility syntax only.
