...
Bare `T[]` is not a normal by-value type. Use `T[]&` for a borrowed runtime-length view over
...
borrowed view. `T[]^`, which is `Box<Slice<T>>`, is the owned unsized array handle. Use
`Vector<T>.into_array()` when the vector should be consumed and its initialized items should become
