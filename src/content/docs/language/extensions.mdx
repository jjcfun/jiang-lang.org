...
Compiler-owned type names are hidden. Extensions target the surface forms directly, including
generic patterns such as `extend <T> T&` and `extend <T> T[]^`.
...
Jiang does not impose a global orphan rule on extensions: a module may extend a public external or
builtin type. Lookup only considers extensions visible from the use site. A concrete target is more
specific than a generic target; if multiple callable extensions with the same specificity match,
the call is ambiguous rather than being selected by declaration order.
