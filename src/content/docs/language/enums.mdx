...
Enums can define instance methods and associated functions without a `self` parameter:
...
In patterns, `ref! Int value` creates a unique mutable borrow. Write `ref Int! value` or
`ref! Int! value` when the resulting reference slot itself must be rebindable.
