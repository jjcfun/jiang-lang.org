...
Use `_&` to borrow a union payload instead of binding it by value:
...
`_&!` is also allowed when the payload reference binding itself must be reassignable; the `!` applies
to the binding, not to the payload source.
...
Borrowed union payloads use the same `_&` sub-pattern:
