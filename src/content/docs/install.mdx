...
Jiang 0.4.8 provides a prebuilt macOS arm64 compiler package. This is the recommended way to try the language today.
...
Download `jiang-0.4.8-macos-arm64.zip` from the [0.4.8 GitHub release](https://github.com/jjcfun/jiang/releases/tag/0.4.8), then unpack it.
...
Build and verify the compiler with the installed 0.4.8 release:

```bash
BOOTSTRAP_RELEASE_VERSION=0.4.8 \
BOOTSTRAP_BIN="$HOME/.jiang/versions/0.4.8/bin/jiangc" \
